(() => {
  ...
  const current = Math.min(Math.max(Number(params.get('chapter')) || 1, 1), essays.length || 10);
  ...
  function renderChapterList() {
    ...
    chapterList.innerHTML = essays.map(essay => `
      <li>
        <a href="${chapterUrl(essay.n)}" ${essay.n === current ? 'aria-current="page"' : ''}>
  ...
  async function loadChapter() {
    ...
    if (meta) meta.textContent = `Essay ${String(essay.n).padStart(2, '0')} of ${essays.length}`;
  ...
  drawerButton?.addEventListener('click', () => {
    ...
    drawerButton.setAttribute('aria-expanded', String(Boolean(open)));
  });
